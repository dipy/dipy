...
""" 
Compute angular error (do not include in demo)
"""
...
shape = csd_sf_orig_slice.shape[:-1]

# noise
csd_noise_angerr = np.zeros(shape)
for idx in ndindex(shape):
	directions = peak_directions(csd_sf_orig_slice[idx], sphere)[0]
	directions_enh = peak_directions(csd_sf_noisy_slice[idx], sphere)[0]
	angerr = 0.0
	for i in directions:
		scores = []
		for j in directions_enh:
			abdot = np.fabs(np.dot(i/np.linalg.norm(i),j/np.linalg.norm(j)))
			if abdot < 1:
				scores.append(180.0/np.pi*np.arccos(abdot))
			else:
				scores.append(0.0)
		angerr += np.amin(scores)
	angerr /= len(directions)
	csd_noise_angerr[idx] = angerr
mean_noise_angerr = np.mean(csd_noise_angerr)
print("average angular error (noise)" + str(mean_noise_angerr))

# enhancements
csd_enh_angerr = np.zeros(shape)
for idx in ndindex(shape):
	directions = peak_directions(csd_sf_orig_slice[idx], sphere)[0]
	directions_enh = peak_directions(csd_sf_enh_slice[idx], sphere)[0]
	angerr = 0.0
	for i in directions:
		scores = []
		for j in directions_enh:
		...
		angerr += np.amin(scores)
	angerr /= len(directions)
	csd_enh_angerr[idx] = angerr
mean_enh_angerr = np.mean(csd_enh_angerr)
print("average angular error (csd + enhancement)" + str(mean_enh_angerr))

# enhancements + sharpening
csd_enh_sharp_angerr = np.zeros(shape)
for idx in ndindex(shape):
	directions = peak_directions(csd_sf_orig_slice[idx], sphere)[0]
	directions_enh = peak_directions(csd_sf_enh_sharp_slice[idx], sphere)[0]
	angerr = 0.0
	for i in directions:
		scores = []
		for j in directions_enh:
		...
		angerr += np.amin(scores)
	angerr /= len(directions)
	csd_enh_sharp_angerr[idx] = angerr
mean_enh_sharp_angerr = np.mean(csd_enh_sharp_angerr)
print("average angular error (csd + enhancement + sharpening)" + str(mean_enh_sharp_angerr))
