...
confidence_interval = np.stack([md_mean - md_95_width/2, md_mean + md_95_width/2], axis=-1)
...
ind = ax1.imshow(md_mean[:, 0, :].T, interpolation='nearest',
...
ind = ax2.imshow(confidence_interval[:, 0, :,0].T, interpolation='nearest',
                 origin='lower', cmap=plt.cm.gray, vmin=vmin, vmax=vmax)

ax3 = fig.add_subplot(1, 3, 3, title=r'MD - 97.5%')
ax3.set_axis_off()
ind = ax3.imshow(confidence_interval[:, 0, :,1].T, interpolation='nearest', origin='lower',
...
Fractional anistropy (FA) on the other hand, is nonlinear in the DTI coefficients.
This means we have to resort to sampling. Here, we'll compare two samples.
"""
...
ax3.set_axis_off()
ind = ax3.imshow(fa_samples[:, 0, :,1].T-fa_samples[:, 0, :,0].T, interpolation='nearest', origin='lower',
                 cmap=plt.cm.gray)
...
"""
The conventional RTOP estimate is merely the mean of a t-distribution [Sjolund2018]_. 
"""

rtop_mean = mapfit.rtop()

"""
RTOP is linear in the MAPMRI coefficients, so we can evaluate the posterior in closed form. 
"""

rtop_percentiles = mapfit.rtop_percentile(np.array([0.25, 0.75]))
rtop_iqr = rtop_percentiles[..., 1] - rtop_percentiles[..., 0] 

# For convenience, there's also an equivalent property: mapfit.rtop_interquartile_range
...
np.random.seed(0)
rtop_sampled_percentiles = mapfit.percentiles(rtop_func, np.array([0.25, 0.75]), n_samples=1000)
rtop_iqr_sampled = rtop_sampled_percentiles[..., 1] - rtop_sampled_percentiles[..., 0]  

plt.hist((rtop_iqr.flatten() - rtop_iqr_sampled.flatten())/rtop_iqr.flatten(),bins=25); # Relative errors

"""
This seems to have something to do with references to self, 
because it works if you define essentially the same function as an instance method:
"""
...
np.random.seed(0)
rtop_sampled_percentiles = mapfit.rtop_percentile_sampled(np.array([0.25, 0.75]))
rtop_iqr_sampled = rtop_sampled_percentiles[..., 1] - rtop_sampled_percentiles[..., 0]  
...
ax3.set_axis_off()
ind = ax3.imshow(rtop_iqr_sampled[:, 0, :].T, interpolation='nearest', origin='lower',
                 cmap=plt.cm.gray, vmin=vmin, vmax=vmax)
