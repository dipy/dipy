...
class AppendTextFlow(Workflow):
    ...
    def run(self, input_files, text_to_append='dipy', out_dir='',
        ...
        parameter as they will be used with inspection to build the command
...
# anything. For the example, it just appends text to an input file.
#
...
# since ``AppendTextFlow`` class is not on an external file but in the current
# file.
