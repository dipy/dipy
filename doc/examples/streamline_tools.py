...
=================================================
Using Dipy to Group, Count and Target Streamlines
=================================================
...
to apply the CsaOdfModel to each white matter voxel and fiber directions for
tracking.
...
We've set `symmetric` to True so that the start and end points are treated the
same and our connectivity matrix will be symmetric.
...
streamlines that pass though each voxel. We'll also need give `density_map` the
dimensions of the image.

"""
lr_superiorfrontal_track = grouping[11, 54]
dm = utils.density_map(lr_superiorfrontal_track, data.shape[:3])
...
trackvis_header['dim'] = data.shape[:3]
...
`affine` and `voxel_size`, which can be used to specify the coordinate system
being used. If neither of these arugments is specified, the streamlines should
be in voxel coordinates. If the `affine` argument is specified any affine
mapping between voxel coordinates and streamline points can be used.  The
`voxel_size` argument is meant to be used with streamlines in "trackvis space"
along with the `voxel_size` filed of the trackvis header.
