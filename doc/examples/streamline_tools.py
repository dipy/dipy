...
# when the tracking exit the white matter.
...
# returns only those streamlines that pass though the ROI. The ROI should be an
# array such that the voxels that belong to the ROI are ``True`` and all other
# voxels are ``False`` (this type of binary array is sometimes called a mask).
# This function can also exclude all the streamlines that pass though an ROI by
...
# above and all the streamlines that pass though that ROI. The ROI is the
...
#
#
# Once we've targeted on the corpus callosum ROI, we might want to find out
...
#        callosum region is a combination of the FreeSurfer labels 251-255. The
#        remaining FreeSurfer labels were re-mapped and reduced so that they
#        lie between 0 and 88. To see the FreeSurfer region, label and name,
#        represented by each value see `label_info.txt` in
#        `~/.dipy/stanford_hardi`.
# .. [#] An affine transformation is a mapping between two coordinate systems
#        that can represent scaling, rotation, sheer, translation and
