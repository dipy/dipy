...
# the signal values a linear fit can be obtained. Another linear fit for
# ``S0`` (bvals < ``split_b_S0`` (default: 200)) follows and ``f`` is estimated
# using $1 - S0_{prime}/S0$. Then a non-linear least-squares fitting is
# performed to fit ``D_star`` and ``f``. If the ``two_stage`` flag is set to
# ``True`` while initializing the model, a final non-linear least squares
# fitting is performed for all the parameters. All initializations for the
# model such as ``split_b_D`` are passed while creating the ``IvimModel``.
...
# of the first and second optimizers are utilized as the initial values for
...
# of microstructure models. This method has been described in further detail in
