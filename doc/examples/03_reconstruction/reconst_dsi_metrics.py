...
###############################################################################
# gtab contains a GradientTable object (gradient information e.g. b-values).
# For example to read the b-values it is possible to write print(gtab.bvals).
#
...
# By default the propagator is divided by its sum in order to obtain a properly
# normalized pdf, however this normalization changes the values of RTOP,
# therefore in order to compare it with the RTOP previously calculated on the
# signal we turn the normalized parameter to false.
