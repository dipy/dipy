...
def cti_design_matrix(gtab1, gtab2): 
	...
	-------
	B: array(N, 43)
	Design matrix or B matrix for the CTI model assuming multiple Gaussian Components
      ...
      B[:,6] = b1 * n1[:,0]**2 * b2 * n2[:,0]**2
      B[:,7] = b1 * n1[:,1]**2 * b2 * n2[:,1]**2
      B[:,8] = b1 * n1[:,2]**2 * b2 * n2[:,2]**2
      B[: ,9] = 2 * (b1*n1[:,0]**2 * b2*n2[:,0] * n2[:,1] + b1*n1[:,0]*n1[:,1] * b2*n2[:,0]**2)
      B[:,10] = 2 * (b1*n1[:,0]**2 * b2*n2[:,0] * n2[:,2] + b1*n1[:,0]*n1[:,2] * b2*n2[:,0]**2)
      B[:,11] = 2 * (b1*n1[:,1]**2 * b2*n2[:,1] * n2[:,0] + b1*n1[:,1]*n1[:,0] * b2*n2[:,1]**2)
      B[:,12] = 2 * (b1*n1[:,1]**2 * b2*n2[:,1] * n2[:,2] + b1*n1[:,1]*n1[:,2] * b2*n2[:,1]**2)
      B[:,13] = 2 * (b1*n1[:,2]**2 * b2*n2[:,2] * n2[:,0] + b1*n1[:,2]*n1[:,0] * b2*n2[:,2]**2)
      B[:,14] = 2 * (b1*n1[:,2]**2 * b2*n2[:,2] * n2[:,1] + b1*n1[:,2]*n1[:,1] * b2*n2[:,2]**2)
      B[:,15] = b1*n1[:,0]**2 * b2*n2[:,1]**2 + b1*n1[:,1]**2 * b2*n2[:,0]**2
      B[:,16] = b1*n1[:,0]**2 * b2*n2[:,2]**2 + b1*n1[:,2]**2 * b2*n2[:,0]**2
      B[:,17] = b1*n1[:,1]**2 * b2*n2[:,2]**2 + b1*n1[:,2]**2 * b2*n2[:,1]**2
      B[:,18] = 2 * (b1*n1[:,0]**2 * b2*n2[:,1]*n2[:,2] + b1*n1[:,1]*n1[:,2] * b2*n2[:,0]**2)
      B[:,19] = 2 * (b1*n1[:,1]**2 * b2*n2[:,0]*n2[:,2] + b1*n1[:,0]*n1[:,2] * b2*n2[:,1]**2)
      B[:,20] = 2 * (b1*n1[:,2]**2 * b2*n2[:,0]*n2[:,1] + b1*n1[:,0]*n1[:,1] * b2*n2[:,2]**2)
      B[:,21] = 4 * (b1*n1[:,0] * n1[:,1] * b2*n2[:,0] * n2[:,1])
      B[:,22] = 4 * (b1*n1[:,0] * n1[:,2] * b2*n2[:,0] * n2[:,2])
      B[:,23] = 4 * (b1*n1[:,1] * n1[:,2] * b2*n2[:,1] * n2[:,2])
      B[:,24] = 4 * (b1*n1[:,0] * n1[:,1] * b2*n2[:,1] * n2[:,2] + b1*n1[:,1]*n1[:,2] * b2*n2[:,0]*n2[:,1])
      B[:,25] = 4 * (b1*n1[:,0] * n1[:,2] * b2*n2[:,1] * n2[:,2] + b1*n1[:,1]*n1[:,2] * b2*n2[:,0]*n2[:,2])
      B[:,26] = 4 * (b1*n1[:,0] * n1[:,1] * b2*n2[:,0] * n2[:,2] + b1*n1[:,0]*n1[:,2] * b2*n2[:,0]*n2[:,1])
      B[:,27] = (b1*b1*n1[:,0]**4 + b2*b2*n2[:,0]**4)/6
      B[:,28] = (b1*b1*n1[:,1]**4 + b2*b2*n2[:,1]**4)/6
      B[:,29] = (b1*b1*n1[:,2]**4 + b2*b2*n2[:,2]**4)/6
      B[:,30] = (2*b1*b1*n1[:,0]**3 * n1[:,1] + 2*b2*b2*n2[:,0]**3 * n2[:,1])/3
      B[:,31] = (2*b1*b1*n1[:,0]**3 * n1[:,2] + 2*b2*b2*n2[:,0]**3 * n2[:,2])/3
      B[:,32] = (2*b1*b1*n1[:,1]**3 * n1[:,0] + 2*b2*b2*n2[:,1]**3 * n2[:,0])/3
      B[:,33] = (2*b1*b1*n1[:,1]**3 * n1[:,2] + 2*b2*b2*n2[:,1]**3 * n2[:,2])/3
      B[:,34] = (2*b1*b1*n1[:,2]**3 * n1[:,0] + 2*b2*b2*n2[:,2]**3 * n2[:,0])/3
      B[:,35] = (2*b1*b1*n1[:,2]**3 * n1[:,1] + 2*b2*b2*n2[:,2]**3 * n2[:,1])/3
      B[:,36] = b1 * b1 * n1[:,0]**2 * n1[:,1]**2 + b2*b2*n2[:,0]**2 * n2[:,1]**2
      B[:,37] = b1 * b1 * n1[:,0]**2 * n1[:,2]**2 + b2*b2*n2[:,0]**2 * n2[:,2]**2
      B[:,38] = b1 * b1 * n1[:,1]**2 * n1[:,2]**2 + b2*b2*n2[:,1]**2 * n2[:,2]**2
      B[:,39] = 2 * b1 * b1 * n1[:,0]** 2*n1[:,1]*n1[:,2] + 2*b2*b2*n2[:,0]**2*n2[:,1]*n2[:,2]
      B[:,40] = 2 * b1 * b1 * n1[:,1]** 2*n1[:,0]*n1[:,2] + 2*b2*b2*n2[:,1]**2*n2[:,0]*n2[:,2]
      B[:,41] = 2 * b1 * b1 * n1[:,2]** 2*n1[:,0]*n1[:,1] + 2*b2*b2*n2[:,2]**2*n2[:,0]*n2[:,1]
      B[:42] = -np.ones(len(b1))
