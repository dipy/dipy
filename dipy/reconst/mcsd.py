...
def multi_shell_fiber_response(sh_order, bvals, wm_rf, gm_rf, csf_rf,
    ...
    for i, bvalue in enumerate(bvals):
        ...
        response[i, 1] = 1 * np.exp(-bvalue * gm_rf[0]) / A
        response[i, 0] = 1 * np.exp(-bvalue * csf_rf[0]) / A
...
def mask_for_response_msmt(gtab, data, roi_center=None, roi_radii=10,
    ...
    if iso_tol is not None:
        ...
        gm_evals_mean = np.mean(np.array([gm_evals[..., 1], gm_evals[..., 2]]),
                                axis=0)
        mask_gm_iso = np.allclose(gm_evals_mean, gm_evals[..., 0], rtol=tol)
        mask_gm *= mask_gm_iso
        print("Sum GM after iso: ", np.sum(mask_gm))
        print("Sum CSF before iso: ", np.sum(mask_csf))
        csf_evals = evals[mask_csf.astype(bool)]
        csf_evals_mean = np.mean(np.array([csf_evals[..., 1], csf_evals[..., 2]]),
                                 axis=0)
        mask_csf_iso = np.allclose(csf_evals_mean, csf_evals[..., 0], rtol=tol)
