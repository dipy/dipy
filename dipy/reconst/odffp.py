...
class OdffpModel(object):
    ...
    def _find_matching_odf_trace(self, input_odf_trace, dict_odf_trace, penalty, matching_attempt=0):

        # Certain memory allocation issues can be handled by invoking this method repeatedly  
        if matching_attempt >= self.MAX_MATCHING_ATTEMPTS:
            return
        ...
        try:
            ...
            return self._find_matching_odf_trace(input_odf_trace, dict_odf_trace, penalty, matching_attempt+1)
...
class OdffpFit(OdfFit):
    ...
    def save_as_fib(self, affine, voxel_size, output_file_name='output.fib.gz'):
        ...
        for i in range(self._dict.max_peaks_num):
            # QA = NQA after major revision in DSI Studio
            fib['fa%d' % i] = fib['fa%d' % i] - np.nanmin(fib['fa%d' % i])  
            fib['fa%d' % i] /= np.maximum(1e-8, np.nanmax(fib['fa%d' % i]))
