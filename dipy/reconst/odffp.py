...
class OdffpModel(object):
    ...
    def __init__(self, gtab, dict_file, 
        ...
        with h5py.File(dict_file, 'r') as mat_file:
            ...
            self._dict['odf'] = np.array(mat_file['odfrot'])
            self._dict['odf_trace'],_ = self._normalize_odf(self._dict['odf'])
            ...
            self._dict['peak_dirs'] = peak_dirs[:,:self._max_peaks_num]
...
#             self._dict_model = np.ravel(mat_file['libopt']['microstruct']) # TODO: convert to str
    ...
    def _normalize_odf(self, odf):
        if self.drop_negative_odf:
            odf = np.maximum(0, odf)
        
        if self.zero_baseline_odf:
    ...
    def fit(self, data, mask=None):
        ...
        for chunk_idx in np.split(range(voxels_num), range(self.max_chunk_size, voxels_num, self.max_chunk_size)):
            ...
            dict_idx[chunk_idx] = np.argmax(np.dot(input_odf_trace, self._dict['odf_trace']), axis=1)
        
            for i, j in zip(range(chunk_size), chunk_idx):
                if self.output_dict_odf:
...
class OdffpFit(OdfFit):
    ...
    def __init__(self, data, tessellation, odf, peak_dirs, dict_idx, odf_dict):
        self._data = data
...
#                 fib['index%d' % i][j] = np.mod(peak_vertex_idx[sorted_i[i]], tessellation_half_size)
        ...
        for i in range(1, max_peaks_num+1):
            index_maps = {
                ...
                'fib%d_De_|_' % i: self._dict['micro'][2,i], 'fib%d_fin' % i: self._dict['micro'][3,i],
