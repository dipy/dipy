...
def test_shore_metrics():
    ...
    for n in range(int((radial_order) / 2 + 1)):
        integral += c_shore[n] * (np.pi**(-1.5) * zeta **
                                  (-1.5) * genlaguerre(n, 0.5)(0)) ** 0.5

    assert_almost_equal(integral, 1.0, 10)

    # test if the integral of the pdf calculated on a discrete grid is equal
    # to one
    ...
    assert_equal(rtop_mt / rtop_shore_signal <
                 1.10 and rtop_mt / rtop_shore_signal > 0.95, True)
