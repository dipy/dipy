...
def test_misaligned_fibers():
    ...
    With the displacement-based formula (Schilling et al. 2019), coherence
    reflects whether fibers align with the spatial direction between neighboring
    voxels, not with each other. A mixed perpendicular configuration produces
    zero coherence because no fiber aligns with any inter-voxel displacement.

    The test verifies that:
    1. Perpendicular fibers show zero coherence
...
def test_coherence_axis_sensitivity():
    """
    Test that coherence is sensitive to fiber orientation relative to the
    spatial displacement direction.

    With the displacement-based formula from Schilling et al. (2019), a fiber
    must align with the inter-voxel displacement vector to contribute to
    coherence. This is the key property that detects wrong b-vector
    orientations: the same coherent fiber field scores differently depending on
    which absolute direction the fibers point relative to spatial neighbors.
    """
    shape = (3, 3, 3)
    fa = np.ones(shape) * 0.8

    # Restrict to Z-direction neighbors only to isolate axis sensitivity
    z_only_offsets = np.array([[0, 0, 1], [0, 0, -1]])

    # Z-aligned fibers: aligned with Z-displacement → coherent
    peaks_z = np.zeros(shape + (3,))
    peaks_z[...] = [0, 0, 1]
    coherence_z = compute_fiber_coherence(peaks_z, fa, neighbor_offsets=z_only_offsets)

    # X-aligned fibers: perpendicular to Z-displacement → not coherent
    ...
    assert coherence_z > coherence_x, (
        "Orientation-sensitive formula should distinguish "
        "fiber axis from displacement axis"
    )
