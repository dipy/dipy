...
def test_laplacian_GCV_higher_weight_with_noise(radial_order=4, time_order=2, rng=None):
    gtab_4d = generate_gtab4D()
    l1, l2, l3 = [0.0015, 0.0003, 0.0003]
    S = generate_signal_crossing(gtab_4d, l1, l2, l3)
    S_noise = add_noise(S, S0=1.0, snr=10, rng=rng)
...
def test_spherical_l1_increases_sparsity(radial_order=4, time_order=2):
    ...
    with warnings.catch_warnings():
        ...
        warnings.filterwarnings(
            "ignore", message="cvxpy optimization resulted in .*", category=UserWarning
        )
        warnings.filterwarnings(
