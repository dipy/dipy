...
def test_wls_and_ls_fit():
    ...
    D_o, extra = ols_fit_tensor(design_matrix, np.repeat(YN, repeats=1e4 + 1, axis=0), return_lower_triangular=True)
    assert extra is None, "OLS fit should not return extra information"
    D_w, extra = wls_fit_tensor(design_matrix, np.repeat(YN, repeats=1e4 + 1, axis=0), return_lower_triangular=True)
...
def test_rwls_rnlls_irls_fit():
    ...
    model = TensorModel(gtab, fit_method="NLLS", weights=np.array([[1.0, 1.0], [0.0, 1.0]]))
