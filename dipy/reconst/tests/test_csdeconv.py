...
def test_r2_term_odf_sharp():
    ...
    with warnings.catch_warnings():
        ...
        sdt_model = ConstrainedSDTModel(gtab, ratio=3 / 15.0, sh_order_max=8)
...
def test_csd_predict(rng):
    ...
    coeff = rng.random(csd_fit.shm_coeff.shape) - 0.5
    coeff[..., 0] = 10.0
    with warnings.catch_warnings():
        warnings.filterwarnings(
            "ignore",
...
def test_default_lambda_csdmodel():
    ...
    for sh_order_max, expected, e_warn in zip(
        ...
        with warnings.catch_warnings(record=True) as w:
            ...
            model_full = ConstrainedSphericalDeconvModel(
                gtab, response, sh_order_max=s_o_m, reg_sphere=sphere
            )
...
def test_csd_superres():
    ...
    with warnings.catch_warnings(record=True) as w:
        ...
        model16 = ConstrainedSphericalDeconvModel(
...
def test_csd_convergence():
    ...
    with warnings.catch_warnings():
        ...
        model_wo_conv = ConstrainedSphericalDeconvModel(
            gtab, (evals[0], 3.0), sh_order_max=8
        )
