        ...
        if self.gqi_vector is None:
            if self.model.method == 'gqi2':
                ...
                self.gqi_vector = np.real(
                    H(np.dot(self.model.b_vector, sphere.vertices.T) *
                        self.model.Lambda))
            if self.model.method == 'standard':
                self.gqi_vector = np.real(
                    np.sinc(np.dot(self.model.b_vector, sphere.vertices.T) *
...
def equatorial_zone_vertices(vertices, pole, width=5):
    ...
    """
    return [i for i, v in enumerate(vertices)
...
def triple_odf_maxima(vertices, odf, width):
    ...
    indmax3 = eqvert[np.argmin(
                [np.abs(np.dot(vertices[indmax2], vertices[p]))
                 for p in eqvert])]
    odfmax3 = odf[indmax3]
    """
