...
class ForecastModel(OdfModel, Cache):
    ...
    def __init__(self,
        ...
        sh_degree : unsigned int,
            an even integer that represent the SH degree (l) of the basis (max 12)
        lambda_lb: float,
        ...
        ...     fm = ForecastModel(gtab, sh_degree=6)
        >>> f_fit = fm.fit(data)
        >>> d_par = f_fit.dpar
        >>> d_perp = f_fit.dperp
        >>> with warnings.catch_warnings():
        ...
        if 0 <= sh_degree <= 12 and not bool(sh_degree % 2):
            self.sh_degree = sh_degree
        else:
            msg = "sh_degree must be a non-zero even positive number "
        ...
        self.rho = rho_matrix(self.sh_degree, self.one_0_bvecs)
        ...
        self.lb_matrix = lb_forecast(self.sh_degree)
        self.lambda_lb = lambda_lb
        self.lambda_csd = lambda_csd
        self.fod = rho_matrix(sh_degree, self.vertices)
    ...
    def fit(self, data):
        ...
        if M_diff is None:
            M_diff = forecast_matrix(
                self.sh_degree,  d_par, d_perp, self.one_0_bvals)
        ...
        n_c = int((self.sh_degree + 1)*(self.sh_degree + 2)/2)
        coef = np.zeros(n_c)
        coef[0] = c0
...
class ForecastFit(OdfFit):
    ...
    def odf(self, sphere, clip_negative=True):
        ...
        """
        if self.rho is None:
            self.rho = rho_matrix(self.sh_degree, sphere.vertices)
    ...
    def predict(self, gtab=None, S0=1.0):
        ...
        M_diff = forecast_matrix(self.sh_degree,
                                 self.d_par,
                                 self.d_perp,
                                 gtab.bvals)

        rho = rho_matrix(self.sh_degree, gtab.bvecs)
        M = M_diff * rho
...
def forecast_matrix(sh_degree,  d_par, d_perp, bvals):
