...
def finalize_match(double[:, ::1] group_best, cnp.npy_intp[:, ::1] group_idx,
    ...
    penalty : float
        ...
        Nonpositive similarities are excluded from the logarithmic score. If a
        voxel has no positive similarity, its highest raw similarity is used.
    num_threads : int, optional
        Number of OpenMP threads. ``None`` uses all available threads.
    ...
    cdef:
        ...
        cnp.npy_intp v, g, best_j, fallback_j
        double score, best_score, best_similarity
        int threads_to_use
    ...
    with nogil, parallel():
        for v in prange(n_vox, schedule='static'):
            ...
            for g in range(n_groups):
                if group_idx[v, g] < 0:
                    continue
                if group_best[v, g] > best_similarity:
                    best_similarity = group_best[v, g]
                    fallback_j = group_idx[v, g]
                if penalty > 0:
                    if group_best[v, g] <= 0:
                        continue
