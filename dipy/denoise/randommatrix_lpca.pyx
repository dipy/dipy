...
@cython.boundscheck(False)
@cython.nonecheck(False)
cpdef int fast_eig(double[:, ::1] a, double[::1] W, double[::1] WORK,
                   int LWORK, int[::1] IWORK, int LIWORK) nogil except -1:
...
def randommatrix_localpca_parallel(arr, patch_extent=0, out_dtype=None, num_threads=None):
    ...
    cdef:
        ...
        # for memory usage in OPENMP loops. We create a temp memory for each
        # slice for each OPENMP thread. There should be a better way to do
        # this.
    ...
    noise_arr = np.zeros([arr.shape[0], arr.shape[1], arr.shape[2]],
                         dtype=np.float64)
    ...
    for k in prange(patch_radius, sizes[2]-patch_radius, nogil=True, schedule=static):
        for j in range(patch_radius, sizes[1]-patch_radius):
            for i in range(patch_radius, sizes[0]-patch_radius):
                ...
                # X^TX or XX^T
                fast_eig(C[k, :, :], W[k,:], WORK[k, :], LWORK, IWORK[k, :],
                         LIWORK)
                ...
                if mm<=nn:
                    ...
                    fast_matvec('n', diag_eigmat[k, :, :], temp2[k, :],
                                temp3[k, :])
                    ...
                    fast_matvec('n', diag_eigmat[k, :, :], temp1[k, :],
                                temp0[k, :])
