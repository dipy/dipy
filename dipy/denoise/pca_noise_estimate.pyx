...
def pca_noise_estimate(data, gtab):
    """ PCA based local noise estimation [Manjon13]_.
    ...
    # We can do that by decomposing the covariance matrix:
    ...
    eta = (2 + snr**2 - (np.pi / 8) *
           np.exp(-0.5 * (snr**2)) * ((2 + snr**2) *
           sps.iv(0, 0.25 * (snr**2)) + (snr**2) *
           sps.iv(1, 0.25 * (snr**2)))**2)
