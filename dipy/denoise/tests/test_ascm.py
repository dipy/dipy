...
def test_ascm_random_noise(rng):
    ...
    assert_(np.var(S0n) < np.var(S0))  # Variance should be reduced
    assert_equal(np.round(S0n.mean()), 100)  # Mean should be preserved
    # Check that the denoised image is smoother (less extreme values)
    assert_(np.abs(S0n.max() - 100) < np.abs(S0.max() - 100))
