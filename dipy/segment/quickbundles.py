...
# from dipy.segment.clusteringspeed import fast_quick_bundles
from dipy.segment.clustering import (Clustering, ClusterMap)
...
class QuickBundles2(Clustering):
    def __init__(self, threshold, metric=None):
        self.threshold = threshold
        self.metric = metric

    def cluster(self, data):
        clusters = quickbundles(data, self.metric, threshold=self.threshold)
        return ClusterMap(clusters, data=data, metric=self.metric)

# class FastQuickBundles2(Clustering):
#     def __init__(self, threshold, metric=None):
#         self.threshold = threshold
#         self.metric = metric

#     def cluster(self, data):
#         clusters = fast_quick_bundles(data, self.metric, threshold=self.threshold)
#         return ClusterMap(clusters, data=data, metric=self.metric)
