...
class ConstantObservationModel(object):
    ...
    def initialize_param_uniform(image, num_classes):
    ...
    def prob_neighborhood(image, seg, beta, self.nclasses):
    ...
    def prob_image(img, self.nclasses, mu, sigmasq, P_L_N):
...
#        return seg
    ...
    def initialize_maximum_likelihood(nloglike):
    ...
    def icm_ising(nloglike, beta, seg):
...
class ImageSegmenter(object):
    ...
    def segment_HMRF(image, nclasses, max_iter):
        
        print("Computing neg-log-likelihood")
        observationmodel.negloglikelihood()
        negll = observationmodel.negloglikelihood(image)
        
        print("Initializing parameters")
        mu, sigmasq = observationmodel.initialize_param_uniform(image, nclasses)
        ...
        for iter in range(max_iter): # Here is where the EM parts come in
            ...
            PLN = observationmodel._prob_neighborhood(image, seg_init, beta, nclasses)
            PLY = observationmodel.prob_image(img, nclasses, mu, sigmasq, PLN)          
            observationmodel.update_param(image, PLY)
            negll = observationmodel.negloglikelihood(image)
