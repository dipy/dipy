    ...
    def predict(
        ...
        *,
        ...
        return_masks : bool, optional
            Whether to return brain masks. The masks are generated from the
            non-background labels after applying ``remove_holes_and_islands``.
            As a result, they can include voxels labeled as background and are
            not strictly the union of the label maps. If ``return_prob`` is
            True, masks are derived from the discrete ``argmax`` segmentation
            and recovered to the original image space, while probability maps
            remain in SynthSeg model space.
        ...
        masks : np.ndarray (...) or (batch, ...) or None
            Predicted brain mask generated from the non-background labels
            after applying ``remove_holes_and_islands``.
            None if return_masks is False.
        """
        ...
        if return_masks:
            warnings.warn(
                "The returned brain masks are generated by applying "
                "remove_holes_and_islands to SynthSeg's prediction. "
                "Therefore, some voxels predicted as background may be "
                "interpreted as brain in the mask. The union of the predicted "
                "label maps is not equal to the brain mask.",
                UserWarning,
                stacklevel=2,
            )
        ...
        for i in range(len(T1)):
            ...
            if not return_prob or return_masks:
                ...
                if not return_prob:
                    labels[i] = temp
                if return_masks:
                    masks[i] = remove_holes_and_islands(temp > 0).astype(np.int32)
