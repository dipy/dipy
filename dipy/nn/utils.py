...
def transform_img(
    ...
    *,
    ...
    min_bounds, max_bounds = get_bounds(
        shape, affine, considered_points=considered_points
    )
    ...
    scales = voxsize / target_voxsize
    init_shape = np.ceil(final_size / scales).astype(int)

    # Calculate how much crop and pad is needed
    # to get to target_voxsize and final_size
    # with the current new_shape and voxsize
    crop_vs = None
    pad_vs = None
    new_image, pad_vs, crop_vs = pad_crop(new_image, init_shape)
    new_image, _ = reslice(new_image, np.eye(4), tuple(voxsize), tuple(target_voxsize))
...
def recover_img(image, params, order=3):
    ...
    new_image, _ = reslice(image, np.eye(4), tuple(target_voxsize), tuple(voxsize))
    if crop_vs is not None and pad_vs is not None:
        new_image = inv_pad_crop(new_image, crop_vs, pad_vs)
    new_image = affine_transform(
        new_image, new_affine, output_shape=ori_shape, order=order
    )
