...
class ExtractB0Flow(Workflow):
    ...
    def run(
        ...
        """
        io_it = self.get_io_iterator()
        for dwi, bval, bvec, ob0 in io_it:
...
class ExtractShellFlow(Workflow):
    ...
    def run(
        ...
        bvectors_files : string
            Path to the bvectors files. This path may contain wildcards to use
            multiple bvectors files at once.
        bvals_to_extract : variable int, optional
        ...
        for dwi, bval, bvec, oshell in io_it:
            ...
            if isinstance(shell_data, np.ndarray):
                save_nifti(oshell, shell_data, affine, hdr=image.header)
                logging.info("b0 saved as {0}".format(oshell))
            elif isinstance(shell_data, list):
                for i, shell in enumerate(shell_data):
                    shell_value = int(output_bvals[indices[i]])
                    save_nifti(
                        oshell.replace(".nii", f"_{shell_value}_{indices[i]}.nii"),
                        shell,
                        affine,
                        hdr=image.header,
                    )
