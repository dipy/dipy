...
class IoInfoFlow(Workflow):
    ...
    def run(
        ...
        for input_path in io_it:
            ...
            if ipath_lower.endswith(".nii") or ipath_lower.endswith(".nii.gz"):
                ...
                self._print_volumetric_information(
                    data,
                    affine,
                    vox_sz,
                    affcodes,
            ...
            if os.path.basename(input_path).lower().find("bval") > -1:
                ...
                self._print_bval_data_information(
            ...
            if os.path.basename(input_path).lower().find("bvec") > -1:
                ...
                self._print_bvec_data_information(
                    bvecs, bvecs_tol, bvec_property_length
                )
            ...
            if extension in [".trk", ".tck", ".trx", ".vtk", ".vtp", ".fib", ".dpy"]:
                ...
                self._print_tractography_information(
                    sft,
                    lengths_mm,
                    lengths,
                    steps,
    ...
    def _print_volumetric_information(
        self, data, affine, vox_sz, affcodes, alignment_space, tab
    ):
        ...
        def _print_voxel_information(_name, _data, _alignment_space, _tab):
            ...
            self._print_stats_information(_data, _alignment_space, _tab)
            self._print_property_information(
        ...
        if data.ndim == 4:
            _print_voxel_information(
                "Data (0th vol)", data[..., 0], alignment_space, tab
            )
    ...
    def _print_bval_data_information(
        self, bvals, b0_threshold, bshell_thr, alignment_space
    ):
    ...
    def _print_bvec_data_information(self, bvecs, bvecs_tol, alignment_space):
    ...
    def _print_tractography_information(
        self, sft, lengths_mm, lengths, step_size, alignment_space, tab
    ):
        ...
        self._print_stats_information(lengths_mm, alignment_space, tab)
        logging.info(f"{TractographyPropertyName.LENGTH_NUM_PTS.value}:")
        self._print_stats_information(lengths, alignment_space, tab)
        logging.info(f"{TractographyPropertyName.STEP_SIZE.value}:")
        self._print_stats_information(step_size, alignment_space, tab)
        self._print_property_information(
