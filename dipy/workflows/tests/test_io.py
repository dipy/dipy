...
def test_extract_b0_flow():
    with TemporaryDirectory() as out_dir:
        ...
        extract_b0_flow.run(fdata, fbval, fbvec, out_dir=out_dir)
...
def test_extract_shell_flow():
    with TemporaryDirectory() as out_dir:
        ...
        res, _ = load_nifti(extract_shell_flow.last_generated_outputs["out_shell"])
        ...
        npt.assert_equal(os.path.isfile(pjoin(out_dir, "shell_0_0.nii.gz")), True)
        npt.assert_equal(os.path.isfile(pjoin(out_dir, "shell_2000_10.nii.gz")), True)
        res0, _ = load_nifti(pjoin(out_dir, "shell_0_0.nii.gz"))
        res2000, _ = load_nifti(pjoin(out_dir, "shell_2000_10.nii.gz"))
        npt.assert_array_equal(res0, data[..., 0])
        npt.assert_array_equal(res2000, data[..., 10])
