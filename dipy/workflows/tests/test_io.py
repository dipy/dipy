...
def test_io_info():
    ...
    io_info_flow = IoInfoFlow()
    io_info_flow.run([str(fimg), str(fbvals), str(fvecs)])

    io_info_flow = IoInfoFlow()
    io_info_flow.run(
        [str(fimg), str(fbvals), str(fvecs)], b0_threshold=20, bvecs_tol=0.001
    )

    filepath_dix, _, _ = get_fnames(name="gold_standard_tracks")
    if not is_big_endian:
        io_info_flow = IoInfoFlow()
        io_info_flow.run(str(filepath_dix["gs.trx"]))

    io_info_flow = IoInfoFlow()
    io_info_flow.run(str(filepath_dix["gs.trk"]))

    io_info_flow = IoInfoFlow()
    npt.assert_raises(SystemExit, io_info_flow.run, str(filepath_dix["gs.tck"]))
    io_info_flow = IoInfoFlow()
    npt.assert_raises(OSError, io_info_flow.run, "fake.vtk")

    io_info_flow = IoInfoFlow()
    io_info_flow.run(str(filepath_dix["gs.tck"]), reference=str(filepath_dix["gs.nii"]))
...
def test_convert_tractogram_flow():
    with TemporaryDirectory() as out_dir:
        ...
        convert_tractogram_flow.run(
            input_files, reference=str(data_path["gs.nii"]), out_dir=out_dir
        )
