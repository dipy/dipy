    ...
    for this_fname in [fbvals, fbvecs]:
        ...
        if this_fname is None:
            ...
            if isinstance(this_fname, string_types):
                ...
                with open(this_fname, 'r') as file_to_search:
                    string_to_search = file_to_search.read()
                    sniffer = csv.Sniffer()
                    text_delimiter = sniffer.sniff(str(string_to_search))
                if ext in ['.bvals', '.bval', '.bvecs', '.bvec', '.txt', '.eddy_rotated_bvecs', '']:
                    vals.append(np.squeeze(np.loadtxt(this_fname, delimiter = text_delimiter)))
                elif ext == '.npy':
