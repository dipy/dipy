...
__all__ = ['axes', 'streamtube', 'line', 'dots', 'point', 'label', 'volume',
           'contour', 'sphere_funcs', 'tensor', 'slicer']
...
def axes(scale=(1, 1, 1), colorx=(1, 0, 0), colory=(0, 1, 0), colorz=(0, 0, 1),
         opacity=1):
    """Create an actor with the coordinate system axes.
...
def streamtube(lines, colors, opacity=1, linewidth=0.15, tube_sides=8,
    ...
    Parameters
    ----------
    lines : list of N ``(M, 3)`` arrays
        List of N curves, each represented as a ``(M, 3)`` ndarray
    colors : array (N, 3) or tuple (3,)
        Colors of the streamtubes.
    opacity : float
        Opacity of the streamtubes.
    linewidth : float
    ...
    therefore, you may experience slow rendering time depending on system
    GPU. A solution to this problem is to reduce the number of points in each
...
def line(lines, colors, opacity=1, linewidth=1):
    ...
    Parameters
    ------------
    lines : list of (N, 3) arrays
        Each entry in the list represents the 3-D coordinates of a line, e.g.
        ``lines = [np.random.rand(10,3), np.random.rand(20,3)]``.
        represents two lines, the first with 10 points and the second with 20
        points, each specified in ``(x, y, z)`` coordinates.
    colors : (N, 3) array
        Colormap where every triplet is encoding red, green and blue, e.g.
...
def dots(points, color=(1, 0, 0), opacity=1, dot_size=5):
    """Create an actor for one or more 3-D points.

    Parameters
    ----------
    points : (N, 3) ndarray
        ``(x, y, z)`` coordinate for each of N points.
    color : tuple (3,)
        Point color, ``(r, g, b)``, with values for each color between
        0 and 1.
    opacity : float
        Opacity of the points (value between 0 and 1).
    dot_size : int
        Size of the points.
...
def volume(vol, voxsz=(1.0, 1.0, 1.0), affine=None, center_origin=1,
    ...
    maptype : int {0,1}
        The maptype is an important parameter which affects the raycasting
    ...
    if affine is not None:
        ...
        aff.DeepCopy((affine[0, 0], affine[0, 1], affine[0, 2], affine[0, 3],
        ...
        if center_origin:
            changeFilter.SetOutputOrigin(-vol.shape[0] / 2.0 + 0.5,
                                         -vol.shape[1] / 2.0 + 0.5,
                                         -vol.shape[2] / 2.0 + 0.5)
    ...
    for i in range(colormap.shape[0]):
        color.AddRGBPoint(colormap[i, 0], colormap[i, 1], colormap[i, 2],
                          colormap[i, 3])
    ...
    if (maptype == 1):
        ...
        if affine is None:
            ...
            # index=im2.FindPoint(vol.shape[0]/2.0,vol.shape[1]/2.0,
            #                     vol.shape[2]/2.0)
...
def _create_colormap(v, name='jet', auto=True):
    '''Create colors from a specified colormap and return it
    as an array of shape ``(N, 3)`` where every row gives the corresponding
    ``(r, g, b)`` value. The colormaps we use are similar to those of pylab.

    Parameters
    ----------
    v : (N,) array
        Vector of values to be mapped in RGB colors according to colormap.
    name : {'jet', 'blues', 'blue_red', 'accent'}
        Name of the colourmap.
    auto : bool
        If `auto` is True then v is interpolated to ``[0, 10]`` from
        ``v.min()`` to ``v.max()``.

    Notes
    -----
    If you want to add more colormaps here is what to do. Go to
    http://www.scipy.org/Cookbook/Matplotlib/Show_colormaps,
    see which colormap you need and then get it from ``pylab`` using the
    ``cm.datad`` dictionary.
    ...
    if name == 'jet':
        ...
        green = np.interp(v, [0, 0.125, 0.375, 0.64, 0.91, 1],
                             [0, 0, 1, 1, 0, 0])
        blue = np.interp(v, [0, 0.11, 0.34, 0.65, 1], [0.5, 1, 1, 0, 0])

    if name == 'blues':
        red = np.interp(
            v, [0.0, 0.125, 0.25, 0.375, 0.5, 0.625, 0.75, 0.875, 1.0],
            [0.96862745285, 0.870588243008, 0.776470601559, 0.61960786581,
             0.419607847929, 0.258823543787, 0.129411771894, 0.0313725508749,
             0.0313725508749])
        green = np.interp(
            v, [0.0, 0.125, 0.25, 0.375, 0.5, 0.625, 0.75, 0.875, 1.0],
            [0.984313726425, 0.921568632126, 0.858823537827, 0.792156875134,
             0.68235296011, 0.572549045086, 0.443137258291, 0.317647069693,
             0.188235297799])
        blue = np.interp(
            v, [0.0, 0.125, 0.25, 0.375, 0.5, 0.625, 0.75, 0.875, 1.0],
            [1.0, 0.96862745285, 0.937254905701, 0.882352948189,
             0.839215695858, 0.776470601559, 0.709803938866, 0.611764729023,
             0.419607847929])
    ...
    if name == 'accent':
        red = np.interp(
            v, [0.0, 0.14285714285714285, 0.2857142857142857,
...
def sphere_funcs(sphere_values, sphere, image=None, colormap='jet',
    ...
    sphere : Sphere
        Directions on which the function was evaluated.
    image : None,
        Not  yet supported.
    colormap : {'jet', 'blues', 'blue_red', 'accent'}, optional
        If set to None, no color is used.
    scale : float,
    ...
    for ijk in np.ndindex(*grid_shape):
        ...
        if colormap is not None:
            cols = _create_colormap(m, colormap)
...
def tensor(evals, evecs, scalar_colors=None, sphere=None, scale=2.2,
           norm=True):
...
def slicer(vol, voxsz=(1.0, 1.0, 1.0), plane_i=[0], plane_j=None,
    ...
    """Return a slicer actor for slicing a 3D volume.
