...
class DiffeomorphicMap(object):
    def __init__(self,
                 dim,
                 disp_shape,
                 disp_grid2space=None,
                 domain_shape=None,
                 domain_grid2space=None,
                 codomain_shape=None,
                 codomain_grid2space=None,
                 prealign=None):
        r""" DiffeomorphicMap
        ...
        disp_grid2space : the voxel-to-space transformation between the deformation field's
            grid and space
        domain_shape : array, shape (dim,)
            the number of slices (if 3D), rows and columns of the default
            discretizatio of this map's domain
        domain_grid2space : array, shape (dim+1, dim+1)
        ...
        codomain_grid2space : array, shape (dim+1, dim+1)
        ...
        self.disp_grid2space = disp_grid2space
        if(self.disp_grid2space is None):
            self.disp_space2grid = None
        else:
            self.disp_space2grid = npl.inv(self.disp_grid2space)
        ...
        self.domain_grid2space = domain_grid2space
        if(domain_grid2space is None):
            self.domain_space2grid = None
        else:
            self.domain_space2grid = npl.inv(domain_grid2space)

        # If codomain shape was not provided, we assume it is an endomorphism:
        # use the same domain_shape and codomain_grid2space as the field domain
        ...
        self.codomain_grid2space = codomain_grid2space
        if codomain_grid2space is None:
            self.codomain_space2grid = None
        else:
            self.codomain_space2grid = npl.inv(codomain_grid2space)
    ...
    def _warp_forward(self, image, interpolation='linear', image_space2grid=None,
                      out_shape=None, out_grid2space=None):
        ...
        image_space2grid : array, shape (dim+1, dim+1)
        ...
        out_grid2space : the transformation bringing voxel coordinates of the
        ...
        if image_space2grid is None:
            image_space2grid = self.codomain_space2grid
        ...
        if out_grid2space is None:
            out_grid2space = self.domain_grid2space

        W = None if image_space2grid == 'identity' else image_space2grid
        Dinv = self.disp_space2grid
        P = self.prealign
        S = None if out_grid2space == 'identity' else out_grid2space
    ...
    def _warp_backward(self, image, interpolation='linear', image_space2grid=None,
                       out_shape=None, out_grid2space=None):
        ...
        if image_space2grid is None:
            image_space2grid = self.domain_space2grid
        ...
        if out_grid2space is None:
            out_grid2space = self.codomain_grid2space
    ...
    def transform(self, image, interpolation='linear', image_space2grid=None,
                  out_shape=None, out_grid2space=None):
        r"""Warps an image in the forward direction
        ...
        if out_shape is not None:
            out_shape = np.asarray(out_shape, dtype=np.int32)
        if self.is_inverse:
            warped = self._warp_backward(image, interpolation, image_space2grid,
                                         out_shape, out_grid2space)
        else:
            warped = self._warp_forward(image, interpolation, image_space2grid,
                                        out_shape, out_grid2space)
        return np.asarray(warped)

    def transform_inverse(self, image, interpolation='linear', image_space2grid=None,
                          out_shape=None, out_grid2space=None):
        r"""Warps an image in the backward direction
    ...
    def inverse(self):
        ...
        inv = DiffeomorphicMap(self.dim,
                               self.disp_shape,
                               self.disp_grid2space,
                               self.domain_shape,
                               self.domain_grid2space,
                               self.codomain_shape,
                               self.codomain_grid2space,
                               self.prealign)
    ...
    def expand_fields(self, expand_factors, new_shape):
        ...
        expanded_grid2space = mult_aff(self.disp_grid2space, 
                                       np.diag(expand_factors))
        expanded_space2grid = npl.inv(expanded_grid2space)
        self.forward = expanded_forward
        self.backward = expanded_backward
        self.disp_shape = new_shape
        self.disp_grid2space = expanded_grid2space
        self.disp_space2grid = expanded_space2grid
    ...
    def warp_endomorphism(self, phi):
        ...
        premult_disp = self.disp_space2grid
    ...
    def get_simplified_transform(self):
        ...
        result, self.prealign, self.disp_grid2space, self.domain_grid2space and
        ...
        D = self.domain_grid2space
        P = self.prealign
        Rinv = self.disp_space2grid
        Cinv = self.codomain_space2grid
        ...
        C = self.codomain_space2grid
        Pinv = self.prealign_inv
        Dinv = self.domain_space2grid
...
class SymmetricDiffeomorphicRegistration(DiffeomorphicRegistration):
    ...
    def update(self, current_displacement, new_displacement,
        ...
        disp_space2grid : array, shape (dim+1, dim+1)
        ...
        # grid2space transform, which implies premult_index=Identity
        # and premult_disp is the space2grid transform associated with
        # the displacements' grid
        self.compose(current_displacement, new_displacement, None,
                     disp_space2grid, time_scaling, current_displacement)
    ...
    def _init_optimizer(self, static, moving,
        ...
        disp_grid2space = self.static_ss.get_affine(self.levels-1)
        ...
        codomain_grid2space = static_grid2space
        ...
        domain_grid2space = static_grid2space
        ...
        domain_grid2space = moving_grid2space
    ...
    def _iterate(self):
        ...
        current_disp_grid2space = \
            self.static_ss.get_affine(self.current_level)
        current_disp_space2grid = \
        ...
        self.metric.set_moving_image(wmoving, current_disp_grid2space,
            current_disp_spacing, self.static_direction)
        self.metric.use_moving_image_dynamics(
            current_moving, self.moving_to_ref.inverse())

        self.metric.set_static_image(wstatic, current_disp_grid2space,
            current_disp_spacing, self.static_direction)
        ...
        self.static_to_ref.forward, md_forward = self.update(
            ...
            current_disp_space2grid, self.step_length)
        del fw_step
    ...
    def optimize(self, static, moving, static_grid2space=None, moving_grid2space=None,
                 prealign=None):
        r"""
