...
@cython.boundscheck(False)
@cython.wraparound(False)
cpdef double compute_energy_SSD2D(floating[:, :] deltaField,
                                  floating[:, :] sigmaField,
                                  floating[:, :, :] gradientField,
                                  double lambdaParam,
                                  floating[:, :, :] displacementField):
...
@cython.boundscheck(False)
@cython.wraparound(False)
cpdef double compute_energy_SSD3D(floating[:, :, :] deltaField,
                                  floating[:, :, :] sigmaField,
                                  floating[:, :, :, :] gradientField,
                                  double lambdaParam,
                                  floating[:, :, :, :] displacementField):
...
@cython.boundscheck(False)
@cython.wraparound(False)
def compute_residual_displacement_field_SSD3D(floating[:, :, :] deltaField,
                                              floating[:, :, :] sigmaField,
                                              floating[:, :, :, :] gradientField,
                                              floating[:, :, :, :] target,
                                              double lambdaParam,
        ...
        for r in range(nrows):
            for c in range(ncols):
                ...
                for k in range(NUM_NEIGHBORS):
                    ...
                    if((dc < 0) or (dc >= ncols)):
                        continue
                    y[0] += displacementField[s, r, c, 0] - \
                        displacementField[ds, dr, dc, 0]
                    y[1] += displacementField[s, r, c, 1] - \
                        displacementField[ds, dr, dc, 1]
                    y[2] += displacementField[s, r, c, 2] - \
                        displacementField[ds, dr, dc, 2]
                if(isinf(sigma)):
                    residual[s, r, c, 0] = -lambdaParam * y[0]
                    residual[s, r, c, 1] = -lambdaParam * y[1]
                    residual[s, r, c, 2] = -lambdaParam * y[2]
                else:
                    dotP = gradientField[s, r, c, 0] * displacementField[s, r, c, 0] + gradientField[s, r, c, 1] * \
                        displacementField[s, r, c, 1] + \
                        gradientField[s, r, c, 2] * \
                        displacementField[s, r, c, 2]
                    residual[s, r, c, 0] = b[0] - \
                        (gradientField[s, r, c, 0]
                         * dotP + sigma * lambdaParam * y[0])
                    residual[s, r, c, 1] = b[1] - \
                        (gradientField[s, r, c, 1]
                         * dotP + sigma * lambdaParam * y[1])
                    residual[s, r, c, 2] = b[2] - \
                        (gradientField[s, r, c, 2]
                         * dotP + sigma * lambdaParam * y[2])
    return residual


@cython.boundscheck(False)
@cython.wraparound(False)
cpdef compute_residual_displacement_field_SSD2D(floating[:, :] deltaField,
                                                floating[:, :] sigmaField,
                                                floating[:, :, :] gradientField,
                                                floating[:, :, :] target,
                                                double lambdaParam,
    ...
    for r in range(nrows):
        for c in range(ncols):
            ...
            for k in range(NUM_NEIGHBORS):
                ...
                if((dc < 0) or (dc >= ncols)):
                    continue
                y[0] += displacementField[r, c, 0] - \
                    displacementField[dr, dc, 0]
                y[1] += displacementField[r, c, 1] - \
                    displacementField[dr, dc, 1]
            if(isinf(sigma)):
                residual[r, c, 0] = -lambdaParam * y[0]
                residual[r, c, 1] = -lambdaParam * y[1]
            else:
                dotP = gradientField[r, c, 0] * displacementField[r, c, 0] + \
                    gradientField[r, c, 1] * displacementField[r, c, 1]
                residual[r, c, 0] = b[0] - \
                    (gradientField[r, c, 0] * dotP +
                     sigma * lambdaParam * y[0])
                residual[r, c, 1] = b[1] - \
                    (gradientField[r, c, 1] * dotP +
                     sigma * lambdaParam * y[1])
    return residual
    ...
    for s in range(nslices):
        for r in range(nrows):
            for c in range(ncols):
                ...
                nrm2 = demonsStep[s, r, c, 0] ** 2 + \
                    demonsStep[s, r, c, 1] ** 2 + demonsStep[s, r, c, 2] ** 2
                if(maxDisplacement < nrm2):
                    maxDisplacement = nrm2
    maxDisplacement = sqrt(maxDisplacement)
    factor = maxStepSize / maxDisplacement
