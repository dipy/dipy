...
def test_combine_bundle():
    with TemporaryDirectory() as in_dir:
        ...
        bundle_obj1 = load_tractogram(
            f"{in_dir}/sub_1/AF_L.trk", reference="same", bbox_valid_check=False
        )
        ...
        bundle_obj2 = load_tractogram(
            f"{in_dir}/sub_2/AF_L.trk", reference="same", bbox_valid_check=False
        )
        ...
        for dist in ["mdf", "mdf_se"]:
            ...
            combined = combine_bundles(
                bundle1, bundle2, "random_pick", dist, n_stream=33
            )
            assert_equal(len(combined), 33)
            combined = combine_bundles(
                bundle2, bundle1, "random_pick", dist, n_stream=33
            )
            assert_equal(len(combined), 33)
