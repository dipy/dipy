...
def test_resample():
    ...
    nb_points = 12
    resampled_streamline = dipystreamline.resample(streamline, nb_points)
    resampled_streamline_python = resample_python(streamline, nb_points)
    assert_equal(len(resampled_streamline), nb_points)
    # Using a 5 digits precision because of streamline is in float32.
    assert_array_almost_equal(resampled_streamline, resampled_streamline_python, 5)

    resampled_streamline = dipystreamline.resample(streamline.astype(np.float64), nb_points)
    resampled_streamline_python = resample_python(streamline.astype(np.float64), nb_points)
    assert_equal(len(resampled_streamline), nb_points)
    assert_array_almost_equal(resampled_streamline, resampled_streamline_python)

    res = []
    streamline = np.array([[0,0,0],[1,1,1],[2,2,2]],'f4')
    for nb_points in range(2,200):
        resampled_streamline = dipystreamline.resample(streamline, nb_points)
        res.append(nb_points - len(resampled_streamline))
