...
def speedup_resample():
    print("Speedtest on resample()")
    streamline = np.random.rand(1000, 3)
    nb_points = 42

    import time
    start = time.time()
    dipystreamline.resample(streamline, nb_points)
    cython_time = time.time() - start
    print("Cython: {0:.2}sec".format(cython_time))

    start = time.time()
    resample_python(streamline, nb_points)
    python_time = time.time() - start
    print("Python: {0:.2}sec".format(python_time))

    print("=> {0}x".format(python_time/cython_time))


def speedup_length():
    print("Speedtest on length()")
    streamline = np.random.rand(1000, 3)
    nb_points = 42

    import time
    start = time.time()
    dipystreamline.length(streamline)
    cython_time = time.time() - start
    print("Cython: {0:.2}sec".format(cython_time))

    start = time.time()
    length_python(streamline, nb_points)
    python_time = time.time() - start
    print("Python: {0:.2}sec".format(python_time))
