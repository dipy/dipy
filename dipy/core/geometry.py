...
def cart2sphere(x, y, z):
    ...
    cos = np.divide(z, r, where=r > 0, out=None)
    theta = np.arccos(cos, where=(cos >= -1) & (cos <= 1), out=None)
...
def generate_unit_determinant_matrix():
    ...
    if det < 0:
        mat = np.concatenate(
            (np.negative(mat[0, :])[np.newaxis, :], mat[1:, :]), axis=0
        )
...
def rand_shear_matrix():
    ...
    shear_xy, shear_xz, shear_yx, shear_yz, shear_zx, shear_zy = np.random.random(
        size=(6,)
    )
