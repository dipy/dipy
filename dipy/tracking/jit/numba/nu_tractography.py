...
from dipy.tracking.jit.numba_njit.num_streamlines_numba import getNumStreamlinesProb_generator
from dipy.tracking.jit.numba_njit.generate_streamlines_numba import genStreamlinesMergeProb_generator
...
class NumbaTracker(GenericJITTracker):
    ...
    def __init__(
        ...
        self.sphere_vertices = np.ascontiguousarray(sphere.vertices, dtype=self.REAL_DTYPE)
    ...
    def get_buffer_size(self) -> int:
        ...
        total_pts = sum(
            ...
            if self.min_steps <= l <= self.max_steps
        )
        ...
        def _yield_slines():
            ...
            for i in range(self.nSlines):
                ...
                if npts < self.min_steps or npts > self.max_steps:
                    print("here")
                    continue
                yield np.asarray(slines[i * step : i * step + npts], dtype=self.REAL_DTYPE)
