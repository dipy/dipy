...
def local_tracker(
    ...
    i = _local_tracker(dg, tc, seed, dir, vs, streamline,
                       step_size, fixedstep, &tissue_type)
    return i, tissue_type
...
cdef int _local_tracker(DirectionGetter dg,
    ...
    for i in range(1, streamline.shape[0]):
        ...
        tissue_type[0] = tc.check_point_c(point)
        if tissue_type[0] == TRACKPOINT:
            continue
        elif (tissue_type[0] == ENDPOINT or
              tissue_type[0] == INVALIDPOINT or
              tissue_type[0] == OUTSIDEIMAGE):
            break
    else:
...
def pft_tracker(
        ...
        np.int_t[:, :]  particle_tissue_types):
    ...
    particle_tissue_types : array, float, (2, particle_count)
        Temporary array for the tissue types of particles.
...
cdef _pft_tracker(DirectionGetter dg,
    ...
    while i < strl_array_len:
        ...
        if tissue_type[0] == TRACKPOINT:
            ...
            continue
        elif tissue_type[0] == INVALIDPOINT:
            if pft_trial < pft_max_trials and i > 1:
                ...
                i = _pft(streamline, i - back_steps, directions, dg, tc,
                         voxel_size, step_size, tissue_type,
    ...
    if tissue_type[0] == OUTSIDEIMAGE or tissue_type[0] == PYERROR:
        i -= 1
    return i
...
cdef _pft(np.float_t[:, :] streamline,
    ...
    for p in range(particle_count):
        ...
        particle_tissue_types[0, p] = TRACKPOINT
        particle_steps[0, p] = 0

    for s in range(pft_nbr_steps):
        for p in range(particle_count):
            if particle_tissue_types[0, p] != TRACKPOINT:
                for j in range(3):
            ...
            if dg.get_direction_c(point, dir):
                particle_tissue_types[0, p] = INVALIDPOINT
                ...
                particle_tissue_types[0, p] = tc.check_point_c(point)
                particle_steps[0, p] = s + 1
                particle_weights[p] *= 1 - tc.get_exclude_c(point)
                if particle_weights[p] < eps:
                    particle_weights[p] = 0
                if (particle_tissue_types[0, p] == INVALIDPOINT and
                        particle_weights[p] > 0):
                    particle_tissue_types[0, p] = TRACKPOINT
        ...
        if sum_weights > 0:
            ...
            if N_effective < particle_count / 10.:
                ...
                for pp in range(particle_count):
                    ...
                    particle_tissue_types[1, pp] = \
                            particle_tissue_types[0, pp]
                ...
                for pp in range(particle_count):
                    ...
                    particle_tissue_types[0, pp] = \
                            particle_tissue_types[1, p_source]
    ...
    tissue_type[0] = <TissueTypes> particle_tissue_types[0, p]
