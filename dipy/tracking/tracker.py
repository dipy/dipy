...
def build_stopping_criterion(mask=None, check_point=None, include=None,
                             exclude=None):
    sc_element = {
        "mask": mask,
        "wm_map": None,
...
def generate_tractogram_py(seeds, streamline_generator=generate_streamlines,
                           postprocess_streamline=None):
    for s in seeds:
        streamline = streamline_generator(s, stopping_criterion,
                                          direction_getter, max_cross,
                                          step_size, maxlen, fixedstep)
        if postprocess_streamline is not None:
            streamline = postprocess_streamline(streamlines)
        yield streamline


def build_tractogram_algorithm(streamline_generator, tracker_func,
                               pmf_gen_func, stopping_criterion_func,
                               postprocess_streamline=None):

    def generate_tractogram(seed_positions, seed_directions,
                            tracker_parameters):
        for s in len(seed_positions):
            streamline = streamline_generator(seed_positions[i],
                                              seed_directions[i],
...
def local_tracking(seed_positons, seed_directions, *,
    ...
    return generate_tractogram_py(seed_positons, seed_directions,
                                  tracker_parameters)


probabilistic_tracking = partial(local_tracking,
                                 tractogram_func=generate_tractogram)
probabilistic_tracking_python = partial(local_tracking,
                                        tractogram_func=generate_tractogram_py)

deterministic_tracking = partial(
    local_tracking, direction_getter="deterministic")
...
if __name__ == "__main__":
    ...
    streamlines = local_tracking(
        p_seed, d_seed, params, tractogram_func=my_generate_tractogram)
