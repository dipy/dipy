...
def test_ptt_tracking():
    ...
    sf = sh_to_sf(
        fod, default_sphere, basis_type="tournier07", sh_order_max=8, legacy=False
    )
    sf[sf < 0] = 0
    ...
    dg_count2 = PTTDirectionGetter.from_pmf(
        sf, sphere=default_sphere, max_angle=20, probe_count=2, probe_radius=0.2
    )
    dg_quality10 = PTTDirectionGetter.from_pmf(
        sf, sphere=default_sphere, max_angle=20, probe_quality=10
    )
    dg_length2 = PTTDirectionGetter.from_pmf(
        sf, sphere=default_sphere, max_angle=20, probe_length=2
    )
    ...
    dg = PTTDirectionGetter.from_pmf(
        np.zeros(sf.shape), sphere=default_sphere, max_angle=20
    )
    streamline_generator = LocalTracking(
        direction_getter=dg,
        step_size=0.2,
