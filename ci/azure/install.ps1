...
if($env:INSTALL_TYPE -match "conda")
{
  ...
  Invoke-Expression "conda install -yq --name testenv $env:DEPENDS $env:EXTRA_DEPENDS pytest nose"
}
else
{
  ...
  Invoke-Expression "$env:PIPI $env:DEPENDS $env:EXTRA_DEPENDS pytest nose"
}