...
def register_FA_same_subj_diff_sessions(dname_grid, dname_shell):
    ...
    tmp_shell = os.path.join(
        tmp_dir, os.path.basename(dname_shell) + '_ref.nii')
    ...
    FA_shell_imgT = dp.volume_transform(
        FA_shell_img, T.inv(), reference=target)
...
def test_registration():
    ...
    sourceT = dp.volume_transform(
        source, T.inv(), reference=target, interp_order=0)
