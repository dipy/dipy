...
eds = np.load(os.path.join(os.path.dirname(dp.__file__), 'core',
              'matrices', 'evenly_distributed_sphere_362.npz'))
...
def run_comparisons(sample_data=35):
    ...
    #    gq =  pkl.load_pickle(gqfile)
        ...
        dt_results = analyze_maxima(
            dt_indices, dt_first_directions_in.reshape((100, 1000, 3)), range(10, 91))
    ...
    for simfile in [simdata[sample] for sample in sample_data]:
        ...
        for j in range(10):
            ...
            gqs = dp.GeneralizedQSampling(
                s.reshape((1, 102)), bvals, gradients, Lambda=7)
