...
def flirt2aff(mat, in_img, ref_img):
    ...
    if npl.det(current_in2ref) < 0:
        raise ValueError(
            'Negative determinant to current affine mapping - bailing out')
...
fdis = d101 + \
    '1312211075232351192010092912092080924175865ep2dadvdiffDSI10125x25x25STs005a001_nonlin_displacements.nii.gz'
ffareg = d101 + \
    '1312211075232351192010092912092080924175865ep2dadvdiffDSI10125x25x25STs005a001_bet_FA_reg.nii.gz'
flirtaff = d101 + \
    '1312211075232351192010092912092080924175865ep2dadvdiffDSI10125x25x25STs005a001_affine_transf.mat'
ftrack = d101 + \
    '1312211075232351192010092912092080924175865ep2dadvdiffDSI10125x25x25STs005a001_QA_native.dpy'
...
dpr.close()
track = T[4]
# ref_name to be replaced by ffareg
im2im = flirt2aff_files(flirtaff, ffa, ref_fname)
print im2im
