...
def rm_far_ends(ref, tracks, dist=25):
    ...
    indices : sequence
       indices of tracks
    '''
    indices = [i for (i, t) in enumerate(tracks)
...
def rm_far_tracks(ref, tracks, dist=25, down=False):
    ...
    if down == False:
        ...
        indices = [i for (i, t) in enumerate(tracksd)
...
def skeletal_tracks(tracks, rand_selected=1000, ball_radius=5, neighb_no=50):
    ...
    # store indices of already visited tracks i.e. which already have a
    ...
    for (i, t) in enumerate(random_indices):
        ...
        if i not in visited:
            ...
            # rm far tracks
            tracksr, indices = rm_far_tracks(
                tracks3points[t], tracks3points, dist=25, down=True)
            ...
            for tri in indices:
                ...
                for p in tracks[t]:
                    ...
                    # if you intersect the sphere surrounding the point of the
                    # random track increase a counter
                    if tm.inside_sphere(tracks[tri], p, ball_radius):
                        cnt_intersected_balls += 1

                # if all spheres are covered then accept this track as your
                # neighbour
            ...
            # if the number of possible neighbours is above threshold then
            # accept track[t] as a representative fiber
...
def detect_corpus_callosum(tracks, plane=91, ysize=217, zsize=181, width=1.0, use_atlas=0, use_preselected_tracks=0, ball_radius=5):
    ...
    for (i, t) in enumerate(tracks):
        ...
        for pi in range(len(t) - 1):
            ...
            # if track segment is cutting the plane (assuming the plane is at
            # the x-axis X=plane)
            if (t[pi][0] <= plane and t[pi + 1][0] >= plane) or (t[pi + 1][0] <= plane and t[pi][0] >= plane):
                ...
                # report the index of the track and the point of intersection
                # with the plane
                cc.append((i, hit))
    ...
    # find the biggest objects the second biggest should be the cc the biggest
    ...
    # now do another dilation to recover some cc shape from the previous
    # erosion
...
def relabel_by_atlas_value_and_mam(atlas_tracks, atlas, tes, tracks, tracksd, zhang_thr):
    ...
    for e in range(1, 9):  # from emi:
        ...
        indices = emi[e]['init_ref'] + \
            emi[e]['selected_ref'] + emi[e]['apr_ref']
        ...
        for value in emi[e]['value']:
            value_indices += track_indices_for_a_value_in_atlas(atlas,
                                                                value, tes, tracks)
        ...
        # check if value_indices do not have anything in common with
        # corresponding_indices and expand
        if list(set(value_indices).intersection(set(corresponding_indices))) == []:
            ...
            for ci in corresponding_indices:
                print 'koukou', ci
                ref = tracksd[ci]
                brain_rf, ind_fr = rm_far_tracks(
                    ref, tracksd, dist=10, down=True)
...
def threshold_hitdata(hitdata, divergence_threshold=0.25, fibre_weight=0.8):
    ...
    firstpass = [[[x, y, z, r, f]
                  for (x, y, z, r, f) in plane if r <= divergence_threshold] for plane in hitdata]
    ...
    heavy_weight_fibres = [
        f for f in fibrecounts.keys() if fibrecounts[f] >= weight_thresh]

    # third pass

    reduced_hitdata = [np.array([[x, y, z, r, f] for (x, y, z, r, f) in plane if fibrecounts[f.astype('int')] >= weight_thresh])
                       for plane in firstpass]
...
def neck_finder(hitdata, ref):
    ...
    # We will assume that hitdata[i] relates to the segment from ref[i] to
    # ref[i+1]
    ...
    for (p, plane) in enumerate(hitdata):
        ...
        radial_distances = np.sqrt(
            np.diag(np.inner(xyz - ref[p], xyz - ref[p])))

        unweighted_mean_rcd += [np.average(1 - rcd)]

        weighted_mean_rcd += [np.average(1 - rcd,
                                         weights=np.exp(-radial_distances))]

        unweighted_mean_dist += [np.average(np.exp(-radial_distances))]

        weighted_mean_dist += [np.average(np.exp(-radial_distances),
                                          weights=1 - rcd)]
...
def max_concentration(plane_hits, ref):
    ...
    '''
    dispersions = [np.prod(np.sort(npla.eigvals(np.cov(p[:, 0:3].T)))[1:2])
                   for p in plane_hits]
...
def refconc(brain, ref, divergence_threshold=0.3, fibre_weight=0.7):
    ...
    reduced_hitdata, heavy_weight_fibres = threshold_hitdata(
        hitdata, divergence_threshold, fibre_weight)
...
def bundle_from_refs(brain, braind, refs, divergence_threshold=0.3, fibre_weight=0.7, far_thresh=25, zhang_thresh=15, end_thresh=10):
    ...
    for ref in refs:
        ...
        refd = tm.downsample(ref, 3)
        brain_rf, ind_fr = rm_far_tracks(
            refd, braind, dist=far_thresh, down=True)
        brain_rf = [brain[i] for i in ind_fr]
        #brain_rf,ind_fr = rm_far_tracks(ref,brain,dist=far_thresh,down=False)
        heavy_weight_fibres, index, centre = refconc(
            brain_rf, ref, divergence_threshold, fibre_weight)
        heavy_weight_fibres_z = [i for i in heavy_weight_fibres if pf.zhang_distances(
            ref, brain_rf[i], 'avg') < zhang_thresh]
...
class FACT_Delta():
    ...
    def __init__(self, qa, ind, seeds_no=1000, odf_vertices=None, qa_thr=0.0239, step_sz=0.5, ang_thr=60.):
        ...
        for i in range(seeds_no):
            ...
            track = self.propagation(
                seed.copy(), qa, ind, odf_vertices, qa_thr, ang_thr, step_sz)
    ...
    def propagation_direction(self, point, dx, qa, ind, odf_vertices, qa_thr, ang_thr):
        ...
        for m in range(8):
            ...
            delta, direction = self.nearest_direction(
                dx, qa_tmp, ind_tmp, odf_vertices, qa_thr, ang_thr)
    ...
    def initial_direction(self, seed, qa, ind, odf_vertices, qa_thr):
        ...
        # corresponing orientation indices for max qa
        ind_tmp = ind[x, y, z, 0]
    ...
    def propagation(self, seed, qa, ind, odf_vertices, qa_thr, ang_thr, step_sz):
        ...
        d, idirection = self.initial_direction(
            seed, qa, ind, odf_vertices, qa_thr)
