...
FA=ten.fa()
FA.shape=FA.shape+(1,)
IN2.shape=IN2.shape+(1,)
T2=tp.FACT_Delta(FA,IN2,seeds_no=10000,qa_thr=0.2).tracks
t6=time.clock()
print ('Create %d FA tracks in %d secs' %(len(T),t6-t5))

T2=[t+np.array([100,0,0]) for t in T2]

print('With red tracks propagated based on QA')
print('With green tracks propagated based on FA')
r=fos.ren()
