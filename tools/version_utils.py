...
def get_version_info(source_root):
    ...
    # write_version_py(), otherwise the import of dipy.version messes
    # up the build under Python 3.
    FULLVERSION = VERSION
    if os.path.exists(os.path.join(source_root, '.git')):
        GIT_REVISION, COMMIT_COUNT = git_version(source_root)
    elif os.path.exists('dipy/version.py'):
        # must be a source distribution, use existing version file
        # load it as a separate module to not load dipy/__init__.py
        import runpy
        ns = runpy.run_path('dipy/version.py')
...
def write_version_py(source_root, filename='dipy/version.py'):
    cnt = """\
